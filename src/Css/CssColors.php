...
final class CssColors
{
    ...
    {
        $hex = self::tryHex($name);
        if (null === $hex) {
            throw new Exception\ParseException(\sprintf('Unrecognized color "%s".', $name));
        }
    ...
    public static function tryParse(string $name): ?SrgbColor
    {
        ...
        return null === $hex ? null : SrgbColor::parseHex('#'.$hex);
    }
}
